...
type
  TMainForm = class(TForm)
    ...
    cbFCOutputTrue: TKRBLComboBox;
    veFCOutputTrueRandSeed: TKRBLValueEdit;
    chFCOutputTrueUseSeed: TKRCheckBox;
...
begin
  ...
  try
    ...
    if chFCOutputTrueUseSeed.Checked then begin
      dw := cfgFCOutputTrueRandSeed.Value;
      rnd.Seed := dw;
    end else begin
      if not QueryPerformanceCounter( i64 ) then i64 := GetTickCount;
      rnd.Seed := ( UInt64( Random( MAXINT ) ) shl 32 ) xor UInt64( i64 );
    ...
    rnd.Init;
    SetLength( yTrue, nn.OutputsCount );
    i := cfgFCOutputTrue.Value;
    case i of
      0: for I := 0 to nn.OutputsCount - 1 do yTrue[ i ] := 0;
      1: for I := 0 to nn.OutputsCount - 1 do yTrue[ i ] := 1;
      2: for I := 0 to nn.OutputsCount - 1 do yTrue[ i ] := rnd.NormalGaussDouble;
      3: for I := 0 to nn.OutputsCount - 1 do yTrue[ i ] := rnd.UniformDouble;
    end;
