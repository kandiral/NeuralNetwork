...
type
  ...
  public
    ...
    procedure Build( AIsTraining: boolean = false ); override;
  end;
...
begin
  ...
  SetLength( FOutGrads, FOutputsCount );
  SetLength( FWeights, FWeightsCount );

  FOutput := @FOutputs[ 0 ];
  FWeight := @FWeights[ 0 ];

  if AIsTraining then begin
    SetLength( FOutGrads, FOutputsCount );
    FOutputGrads := @FOutGrads[ 0 ];
    SetLength( FWeightsGrads, FWeightsCount );
    FWeightGrads := @FWeightsGrads[ 0 ];
    if FUseBiases then begin
      SetLength( FBiasesGrads, FOutputsCount );
      FBiasGrads := @FBiasesGrads[ 0 ];
    end;
  end else begin
    SetLength( FOutGrads, 0 );
    SetLength( FWeightsGrads, 0 );
    SetLength( FBiasesGrads, 0 );
  end;
